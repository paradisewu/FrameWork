...
public delegate void OnDownloadBack(object data);
public class Downloader
{
    ...
    public void UpdateDownload()
    {
        ...
        if (data.www == null)
        {
            ...
            m_fCurProcess = 0;
            //ILog.Debug("start load assetPackeage:" + strWgetUrl);
            //send message:a bundle is download start.
        }
        if (data.www.error != null)
        {
            //ILog.Debug("data.www.error:" + data.strServerUrl);
            //send message: a bundle download error.
            RestartCurUrl();
        }
        else if (data.www.isDone && data.www.progress == 1)
        {
            //ILog.Debug("load assetPackeage succeeded:" + data.strServerUrl);
            m_fCurProcess = 0;
            switch(data.type)
            {
    ...
    public void RestartCurUrl()
    {
        ...
        {
            m_nCurCount += 1;
            if (data.failBack != null) data.failBack.Method.Invoke(data.failBack.Target, new object[] { data.strFile });
            data = null;
            return;
        }
    ...
    public bool StartDownload(List<string> servers, string strPath, string strFile, object data, eDownloadType type = eDownloadType.Type_Texture, 
...
//         fstream.BeginWrite(bytes, 0, bytes.Length, EndWriteCallback, asyncState);
    ...
    public void OnTextureComplete(DownLoadData _data)
    {
        ...
        _data.reBack.Method.Invoke(_data.reBack.Target, new object[] { texture });

        //Texture2D texture2d = texture as Texture2D;
        //byte[] bytes = texture2d.EncodeToJPG();
        //string path = _data.strPersistentPath;//ResPathMgr.GetLocalPersistentAssetPath((string)_data.data, _data.strFile);
        //CreateFile(path, bytes, null);
    }
